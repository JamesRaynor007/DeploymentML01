...
def get_base_url(request: Request):
    # Generar URL base dependiendo del esquema y host de la solicitud
...
def recomendar_peliculas(data, titulo, num_recomendaciones=5):
    ...
    count_matrix = count_vectorizer.fit_transform(data['TokensLista'])
    ...
    recomendaciones_finales = recomendaciones_finales[recomendaciones_finales['title'].str.lower() != titulo.lower()]
    ...
    recomendaciones_finales = recomendaciones_finales[['title', 'vote_average']].head(num_recomendaciones)

    return recomendaciones_finales[['title', 'vote_average']].sort_values(by=["vote_average"], ascending=False)
